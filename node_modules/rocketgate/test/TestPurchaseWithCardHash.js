...
describe("purchase with card hash", function() {
  this.timeout(0);
  it("should perform purchase with card hash", function(done) {
    ...
    service.PerformPurchase(request, response, function(status) {
      ...
      request.Set(GatewayRequest.AMOUNT, "19.99");
      service.PerformPurchase(request, response, function(status) {
        status.should.not.equal(null);
        response.Get(GatewayResponse.RESPONSE_CODE).should.equal(0);
        done();
